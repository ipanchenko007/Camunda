...
class wsBase
{
    ...
    public function __construct ($params = null)
    {
		...
		require_once('logger.php'); //IPanchenko For custom log
    }
    ...
    public function addCaseNote($caseUid, $processUid, $taskUid, $userUid, $note, $sendMail = 1)
    {
        try {
			...
			//BPMS-2354 IPanchenko Если по требованиям PM нет доступа, то проверяем кастом таблицу
            if ($respView["CASES_NOTES"] == 0 && $respBlock["CASES_NOTES"] == 0) {
				//BPMS-2354 IPanchenko Контроль наличия записи в X_APP_PARTICIPATED_USERS
				$sql = "SELECT 1 FROM X_APP_PARTICIPATED_USERS WHERE APP_UID = '$caseUid' AND USR_UID = '$userUid' limit 1";
				//vipLog("class.wsBase.php addCaseNote checkVipPermission sql: $sql");

				try {
					$resVip = executeQuery($sql);
				}
				catch (Exception $e) {
					vipLog("class.wsBase.php addCaseNote checkVipPermission SELECT from X_APP_PARTICIPATED_USERS Error 'e': ".$e->getMessage());
				};

				if (is_array($resVip) && count($resVip) > 0) {
					$userParticipated = 1; //User участвовал, т.к. есть в X_APP_PARTICIPATED_USERS
